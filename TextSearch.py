...
# Define constants for connection string and container name
connection_string = 'DefaultEndpointsProtocol=https;AccountName=kbhdocumentstorage;AccountKey=doSuaslyxCWTQRhiKeyTQEIaT+wVsx4upRJmmNOicvGcb5vJCb1S5d+0bsNQitQxI4uVbYtTwcT1+AStUfrp0Q==;EndpointSuffix=core.windows.net'
container_name = 'kbhdocumentcontainer'
...
def main(action, query_str=None):
    if action == "create_index":
        create_index_and_upload(connection_string, container_name)
    elif action == "search":
        with tempfile.TemporaryDirectory() as temp_index_dir:
            results = search_index(query_str, connection_string, container_name, temp_index_dir)
        ...
        print("Invalid action. Use 'create_index' or 'search'.")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python app.py <action> [<query>]")
        print("Actions: 'create_index' or 'search'")
    else:
        action = sys.argv[1]
        query_str = sys.argv[2] if len(sys.argv) > 2 else None
        main(action, query_str)
